...
# read in data, remove metadata rows, rename columns
...
# import plot background image
lake_image <- readPNG(here("data", "Mono_Lake_from_Mount_Dana_(1).png"))

# create graph of lake level change from 1850 - 2017, label important milestones
ggplot(data = mono, aes(x = year, y = lake_level)) +
  ...
  geom_line() +
  geom_hline(yintercept = 6392, linetype = "dashed", color = "red") +
  geom_hline(yintercept = 6377, linetype = "dashed", color = "orange") +
  geom_hline(yintercept = 6360, linetype = "dashed", color = "grey") +
  annotate(
  ...
  diverting water", size = 3, hjust = "right", color = "white") +
  annotate(geom = "point", x = 1941, y = 6417, color = "yellow", size = 2, alpha = 0.5) +
  annotate(
    geom = "curve", x = 1963, y = 6370, xend = 1979, yend = 6373.4, 
    curvature = .1, arrow = arrow(length = unit(2, "mm")),
    color = "white"
  ) +
  annotate(geom = "text", x = 1962, y = 6370, label = "MLC sues DWP", size = 3, hjust = "right", color = "white") +
  annotate(geom = "point", x = 1979, y = 6373.4, color = "yellow", size = 2, alpha = 0.5) +
   annotate(
  ...
  annotate(geom = "text", x = 1971, y = 6395, label = "Supreme Court rules
           in favor of MLC", size = 3, hjust = "left", color = "white") +
  annotate(geom = "point", x = 1983, y = 6378.6, color = "yellow", size = 2, alpha = 0.5) +
   annotate(
    geom = "curve", x = 1985, y = 6365, xend = 1994, yend = 6374.6, 
    curvature = .3, arrow = arrow(length = unit(2, "mm")),
    color = "white"
  ) +
  annotate(geom = "text", x = 1985, y = 6364.9, label = "Decision 1631", size = 3, hjust = "right", color = "white") +
  annotate(geom = "point", x = 1994, y = 6374.6, color = "yellow", size = 2, alpha = 0.5) +
  annotate(geom = "text", x = 1850, y = 6394, label = "Stable Level", size = 3.5, hjust = "left", color ="red") +
  annotate(
    geom = "curve", x = 2005, y = 6370, xend = 1998, yend = 6384.3, 
    curvature = .1, arrow = arrow(length = unit(2, "mm")),
    color = "white"
  ) +
  annotate(geom = "point", x = 1998, y = 6384.3, colour = "yellow", size = 2, alpha = 0.5) +
  annotate(geom = "text", x = 2011, y = 6366.9, label = "Restoration orders
issued by state", size = 3, hjust = "center", color = "white") +
  annotate(geom = "text", x = 1850, y = 6379, label = "Land bridges emerge", size = 3.5, hjust = "left", color ="orange") +
  annotate(geom = "text", x = 1850, y = 6362, label = "Salinity 120 g/L", size = 3.5, hjust = "left", color ="grey") +
    labs(x = "Year",
         y = "Lake Level (ft above sea level)",
         title = "Mono Lake level changes (1850 - 2017)")
